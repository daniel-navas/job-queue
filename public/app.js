...
function render() {
  ...
  const jobs = data.jobs.filter(j => (active === 'all' || (j.status === active && availableInQueue(j))) && (!discoveryFilter || j.discoveries?.some(d => d.search.provider === discoveryFilter.provider && d.search.id === discoveryFilter.id && d.search.revision === discoveryFilter.revision)) && `${j.reference} ${j.id} ${j.title} ${j.company} ${j.location}`.toLowerCase().includes(query)).sort(compareJobs);
